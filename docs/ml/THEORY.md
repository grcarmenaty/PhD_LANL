...
start) and `results/RESULTS.md` (numerical findings).
...
`results/models/`.  Every trial's metrics, hyperparameters and
runtime are logged to `results/hpo/<task>__<model>__<feature>.json`
so that response surfaces can be plotted post-hoc.

Response-surface plots in `results/figures/hpo/` are generated by
