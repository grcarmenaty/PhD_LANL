...
def _train_one_cell(backbone_name: str, feature: str, task_name: str,
    ...
    X = load_feature(syn_path, feature, normalize=True)
    ...
    y = y_pool
    # Re-derive a seed-dependent subsample so the three seeds see different
    # training subsets (and the split below differs too via make_split).
    if args.subsample is not None and args.subsample < len(np.where(mask)[0]):
        full_pool_idx = np.where(mask)[0]
        rng = np.random.default_rng(20260518 + int(args.seed))
...
def main():
    ...
    p.add_argument("--seed", type=int, default=42,
                      help="Random seed: drives torch/numpy init, the "
                              "synth subsample and the train/val/test split "
                              "so multi-seed runs differ.")
    p.add_argument("--skip-exp-eval", action="store_true",
                      help="Skip the per-cell zero-shot experimental "
                              "evaluation. Use when a separate per-case "
                              "eval pass (eval_vision_percase) will run "
                              "afterwards, to avoid scoring the 2638-case "
                              "set twice per cell.")
    p.add_argument("--per-case-out", type=Path, default=None,
                      help="If set, write per-case experimental predictions "
                              "for each trained cell directly to this "
                              "directory (streaming mode). The per-case JSON "
                              "becomes the resume unit and the model is never "
                              "kept in memory after scoring.")
    p.add_argument("--no-save-model", action="store_true",
                      help="Do not persist the .pt artefact (disk-light "
                              "streaming). Only valid with --per-case-out.")
    args = p.parse_args()

    # Seed everything so the three seeds produce genuinely different runs.
    import random as _random
    import ml_pipeline.train as _train_mod
    _random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    _train_mod.SEED = int(args.seed)   # make_split reads this module global

    print(f"vision sweep [seed={args.seed}]: {len(args.backbones)} backbones x "
    ...
    # Streaming mode: load the experimental context once and write a
    # per-case JSON per cell; the per-case JSON is the resume unit.
    stream = args.per_case_out is not None
    if stream:
    ...
    for backbone in args.backbones:
        for feature in args.features:
            for task in args.tasks:
                ...
                # Resume unit in streaming mode is the per-case JSON.
                if stream:
                    pc_path = args.per_case_out / f"{tag}.json"
                    if pc_path.exists() and not args.force:
                        print(f"  skip {tag} (per-case exists)", flush=True)
                        continue
                if art.exists() and not args.force:
                    ...
                    adapter = blob.get("channel_adapter", "timm_in_chans")
                ...
                # Streaming: write per-case JSON now, free the model, move on.
                if stream:
                    write_per_case(mdl, task, backbone, feature,
                                    blob["n_out"], blob["n_channels"],
                                    args.exp, e_tasks, exp_names,
                                    args.per_case_out,
                                    synth_test=blob.get("test_metric",
                                                        float("nan")),
                ...
                # Cross-domain eval (zero-shot). Skipped when a dedicated
                # per-case eval pass will run afterwards (avoids double work).
                if args.skip_exp_eval:
                    print(f"    [skip-exp-eval] trained {tag}", flush=True)
                    continue
