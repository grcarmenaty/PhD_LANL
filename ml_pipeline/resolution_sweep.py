...
def main() -> None:
    ...
    for hpo_path in sorted((args.out / "hpo").glob("*.json")):
        ...
        if blob["model"] in ("rf", "xgb") and blob["feature"].startswith("cfdac"):
            continue
        # `cnn` (Conv1DStack) and `transformer` (Conv1D projection) both
        # require [N, C, L] input but the sweep feeds them the raw 4D
        # CFDAC tensor [N, 1, H, W] — they FAIL immediately.  Pre-existing
        # bug; cnn2d covers the 2D-input case.  Skip in the plan so we
        # don't burn a 10-min h5 batch_read on the way to the FAIL.
        if blob["model"] in ("cnn", "transformer") and blob["feature"].startswith("cfdac"):
            continue
        if key not in seen:
